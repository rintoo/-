...
f.decomp <- function(coefs, dt_modAdstocked, x, y_pred, i) {
  ...
  xDecompOutAggMeanNon0coefy <- sapply(xDecompOut[, c("intercept", indepVarName), with =F], function(x) sum(x[x>0]))
  xDecompOutAggMeanNon0[is.nan(xDecompOutAggMeanNon0)] <- 0
  xDecompOutAggMeanNon0Perc <- xDecompOutAggMeanNon0/mean(xDecompOutAggMeanNon0)
  ...
  decompOutAgg <- cbind(coefsOut, data.table(xDecompAgg = xDecompOutAgg
                                             ...
                                             ,xDecompOutAggMeanNon0coefy=xDecompOutAggMeanNon0coefy
  ...
  sysTimeDopar <- system.time({
    for (lng in 1:iterNG) {
      ...
      doparCollect <- foreach (
        ...
        dt_decompSpendDist <- decompCollect$xDecompAgg[rn %in% set_mediaVarName, .(rn, xDecompPerc, xDecompMeanNon0Perc, xDecompMeanNon0,xDecompOutAggMeanNon0coefy)]
...
f.robyn <- function(set_hyperBoundLocal
  ...
  decompSpendDist[, y_roi := xDecompOutAggMeanNon0coefy/total_spend]
  decompSpendDist[, roi := xDecompMeanNon0/mean_spend]
  
  setkey(xDecompAgg,solID, rn)
  setkey(decompSpendDist,solID, rn)
  xDecompAgg <- merge(xDecompAgg,decompSpendDist[, .(rn, solID, total_spend, mean_spend, spend_share, effect_share, roi, y_roi)], all.x=TRUE)
